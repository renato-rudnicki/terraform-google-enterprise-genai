...
1. The next instructions assume that you are at the same level of the `terraform-google-enterprise-genai` folder. Create and change into `gcp-network` folder, copy `3-networks-dual-svpc` content, the Terraform wrapper script and ensure it can be executed. Also, initialize git so you can manage versions locally.

   ```bash
   mkdir gcp-network
   cp -R terraform-google-enterprise-genai/3-networks-dual-svpc/* gcp-network
   cp terraform-google-enterprise-genai/build/tf-wrapper.sh gcp-network/
   cp terraform-google-enterprise-genai/.gitignore gcp-network/
...
1. Checkout `nonproduction` and merge `development` into it. Run `init` and `plan` and review output for environment non-production.
