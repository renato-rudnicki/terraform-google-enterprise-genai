...
1. The next instructions assume that you are at the same level of the `terraform-google-enterprise-genai` folder. Create and change into `gcp-projects` folder, copy the code, Terraform wrapper script and ensure it can be executed.

   ```bash
   mkdir gcp-projects
   cp -R  terraform-google-enterprise-genai/4-projects/* gcp-projects/
   cp terraform-google-enterprise-genai/build/tf-wrapper.sh gcp-projects/
   cp terraform-google-enterprise-genai/.gitignore gcp-projects/
...
1. Checkout `non-production` and merge `development` into it. Run `init` and `plan` and review output for environment non-production.
